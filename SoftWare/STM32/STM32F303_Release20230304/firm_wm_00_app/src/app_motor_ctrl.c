...
void app_MotorContrl(void)
{
	...
	switch(motorCtl.motorCtrlStatus)
	{
		...
		case MotorCtrlStatus_EN_StopToMiddle:
			motorCtl.runEn = TRUE;//使能电机
			motorCtl.runPwmFre = motorCtl.runPwmFreFixed / 6;//以1/6的固定频率转动 找中间限位
			if(FALSE == motorCtl.middleAppear)
			{
				motor_pulse_output_set(TRUE);//开始PWM的timer计数 开启PWM 电机运行
				//
				if(TRUE == pos_EventGet(MOTOR_POS_Middle))//通过限位开关判断
				{
					...
					{
						middleAppearDelay = 150;//延迟150ms 电机大概停中间
					}
				}
				if(middleAppearDelay > 0)
				{
					middleAppearDelay--;
					if(0 == middleAppearDelay)
					{
						motorCtl.middleAppear = TRUE;
					}
				}
			}
			else
		...
		case MotorCtrlStatus_EN_StopToMiddle1:	  
			if((FALSE == motorCtl.middleAppear) && (TRUE == pos_EventGet(MOTOR_POS_Middle)))//通过限位开关判断
			{
				pos_EventClear(MOTOR_POS_Middle);				  
				if(SYS_POS_VALUED_1 == pos_FilterGet(MOTOR_POS_Middle))//中间限位
				{
			...
			}else if((TRUE == motorCtl.middleAppear) && (--middleAppearDelay == 0))
			{
