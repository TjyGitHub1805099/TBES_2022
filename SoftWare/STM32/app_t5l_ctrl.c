		...
		if(GET_SDWE_CUR_CYCLE_DATA(SDWE_CYCLE_DATA_PERCENT) >= 100)
		{
			...
			else
			{
				//当采集完成间隔播报时间：大于0时，运行间隔播报
					...
					if(sampleCompletVoiceCyclePrintfTick++ >= SCREEN_SDWE_SAMPLE_COMPLETE_VOICE_PRINTF_OFFSET)//间隔播报一次
					{
...
#define LIUSU_WEIGHT_FILTER_NUM	(30)
//流速计算
void screenSDWe_LiuSuCaculate(UINT32 cycleAvg,UINT32 cycleCurFast,UINT32 cycleCurSlow)
{
	UINT16 i = 0 ;
	static UINT32 u32_TickLiuSu = 0 ,u32_TickLiuSuArray[6];
	static float f_WeightArray[LIUSU_WEIGHT_FILTER_NUM] = {0};
	static float f_WeightDiffArray[LIUSU_WEIGHT_FILTER_NUM] = {0};
	static float f_WeightDiffTotal = 0;
	static UINT16 u16_ArrIndex = 0;
	UINT16 u16_SampleTime = 0;

	static float f_WeightMiddle[6],f_liusu[2],liusu;
	static UINT16 exchange = 0 , lsCaculateStatus = 0 , cycleArrive = 0 ,judgeVlu, index=0;

	static UINT32 avgCaculateTick = 0;
	...
	else
	{
			...
			(SDWeZhanTingYuXing_ZhanTing == GET_SDWE_CUR_CYCLE_DATA(SDWE_CYCLE_DATA_STATUS_ZT_YX)))// /暂停/运行控键 = 暂停（代表正在运行）
		{
			u32_TickLiuSu++;
			#if 0
			u16_SampleTime = cycleCurFast/LIUSU_WEIGHT_FILTER_NUM;
			//
			if((u32_TickLiuSu)%u16_SampleTime == 0)
			{
				f_WeightDiffTotal -= f_WeightDiffArray[u16_ArrIndex];
				//float 重量记录 除去空袋
				f_WeightArray[u16_ArrIndex] = hx711_getWeight(HX711Chanel_1) - (float)GET_SDWE_CUR_CYCLE_DATA(SDWE_CYCLE_DATA_KONGDAI_WEIGHT);
				//计算每次的差值
				if(0 == u16_ArrIndex)
				{
					f_WeightDiffArray[0] = 	f_WeightArray[0] - f_WeightArray[LIUSU_WEIGHT_FILTER_NUM - 1];
				}
				else
				{
					f_WeightDiffArray[u16_ArrIndex] = f_WeightArray[u16_ArrIndex] -	f_WeightArray[u16_ArrIndex-1];
				}
				//计算每次的流速
				f_WeightDiffArray[u16_ArrIndex] *= 1000;
				f_WeightDiffArray[u16_ArrIndex] /=  u16_SampleTime;// g/s
				f_WeightDiffArray[u16_ArrIndex] *=60;//g/min
				//计算流速总量
				f_WeightDiffTotal += f_WeightDiffArray[u16_ArrIndex];
				liusu = f_WeightDiffTotal/LIUSU_WEIGHT_FILTER_NUM;
				//
				u16_ArrIndex++;
				//达到总采样窗口
				if(u16_ArrIndex >= LIUSU_WEIGHT_FILTER_NUM)
				{
					u16_ArrIndex = 0 ;
					if( g_T5L.liusuCur < 0)
					{
					...
					{
						//screenSDWe_LiuSuErrorCaculate((UINT16)g_T5L.liusuCur);
					}
				}
			}
			//
			#else
			switch(lsCaculateStatus)
			{
				case 0xFE://准备开始采样点的初始值
					if(SYS_POS_VALUED_1 == pos_FilterGet(MOTOR_POS_Middle))
					{
						judgeVlu = SYS_POS_INVALUED_0;
					}
					else
					{
						judgeVlu = SYS_POS_VALUED_1;
					}
					lsCaculateStatus = 0xFD ;
					index = 0;
				break;

				case 0xFD://记录6次
					if(judgeVlu == pos_FilterGet(MOTOR_POS_Middle))//通过限位开关判断
					{
						f_WeightMiddle[index] = hx711_getWeight(HX711Chanel_1) - (float)GET_SDWE_CUR_CYCLE_DATA(SDWE_CYCLE_DATA_KONGDAI_WEIGHT);
						u32_TickLiuSuArray[index] = u32_TickLiuSu;
						judgeVlu = (judgeVlu+1)&0x01;
						index++;
						if(index >= 6)
						{
							index = 0 ;
							lsCaculateStatus = 0xFC ;
						}
					}
				break;
					
				case 0xFC:
					f_liusu[0] = f_WeightMiddle[4] - f_WeightMiddle[0];
					f_liusu[0] *=1000;
					f_liusu[0] /=(u32_TickLiuSuArray[4]-u32_TickLiuSuArray[0]);
					f_liusu[0] *=60;

					f_liusu[1] = f_WeightMiddle[5] - f_WeightMiddle[1];
					f_liusu[1] *=1000;
					f_liusu[1] /=(u32_TickLiuSuArray[5]-u32_TickLiuSuArray[1]);
					f_liusu[1] *=60;

					g_T5L.liusuCur = (f_liusu[1] + f_liusu[0])/2;

					screenSDWe_LiuSuErrorCaculate(g_T5L.liusuCur);

					lsCaculateStatus = 0xFE ;
		...
		{
			if(0 != u32_TickLiuSu)//clear data
			{
				u32_TickLiuSu = 0 ;
				f_WeightDiffTotal = 0 ;
				for(i=0;i<LIUSU_WEIGHT_FILTER_NUM;i++)
				{
					f_WeightArray[i] = 0 ;
					f_WeightDiffArray[i] = 0 ;
				}
				g_T5L.liusuCur = 0 ;
				g_T5L.liusuAvg = 0 ;
				u16_ArrIndex = 0;
		...
		if(u32_TickLiuSu != 0)
		{
			g_T5L.liusuAvg = hx711_getWeight(HX711Chanel_1) - (float)GET_SDWE_CUR_CYCLE_DATA(SDWE_CYCLE_DATA_KONGDAI_WEIGHT);
			g_T5L.liusuAvg *=1000;
			g_T5L.liusuAvg /= u32_TickLiuSu;
			g_T5L.liusuAvg *=60;
		}
