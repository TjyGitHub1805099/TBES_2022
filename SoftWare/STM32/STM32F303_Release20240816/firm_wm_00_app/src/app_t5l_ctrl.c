...
{
	static UINT16 bobaoOffset_ticks;
	static UINT8 needComeBackHandle = 0;
	static UINT16 bobaoOffset = 500;//500ms
	static UINT16 beefOffset = 1500;//1500ms 在语音播报完成后播放蜂蜜器
	  ...
	  (TRUE == gSystemPara.u32_CaiJiWanChengBoBaoShengGuangKongEn))
	{
		...
		bobaoOffset_ticks++;
		//
		if(bobaoOffset_ticks % beefOffset == 0)
		{
			sendBeefVoice_Cycle(50);//50*10ms
		}
	}
	else
	{
