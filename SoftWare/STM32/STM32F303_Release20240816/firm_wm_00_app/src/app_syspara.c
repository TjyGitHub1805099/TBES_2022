...
void readSysDataFromFlash_3030(void)
{
	...
	if(crc == cal_crc16(((UINT8 *)&readflashDataBuf[0].u_value),(4*(FLASH_SYS_PARA_STORE_MAX_LEN-1)))) 
	{
		...
		//==202206xx：添加相关参数
		gSystemPara.yusheWeight[SDWeCaiJiDangWei_200] = readflashDataBuf[start_i++].i_value;/**< 200 档预设重量 */
		gSystemPara.yusheWeight[SDWeCaiJiDangWei_300] = readflashDataBuf[start_i++].i_value;/**< 300 档预设重量 */
		gSystemPara.yusheWeight[SDWeCaiJiDangWei_400] = readflashDataBuf[start_i++].i_value;/**< 400 档预设重量 */

		gSystemPara.u16_ml_g_bilv= readflashDataBuf[start_i++].i_value;//ml与g比率
		gSystemPara.u16_fmqkqsj= readflashDataBuf[start_i++].i_value;//蜂鸣器开始时间
		gSystemPara.u16_liusu_high_warn= readflashDataBuf[start_i++].i_value;//流速高 报警点
		gSystemPara.u16_liusu_low_warn= readflashDataBuf[start_i++].i_value;//流速低 报警点

		motorCtl.motorAge = readflashDataBuf[start_i++].i_value;//摇摆角度
		motorCtl.tAD = readflashDataBuf[start_i++].i_value;//摇摆时间
		gSystemPara.u16_kaiqi_guanjiafa_gongneng = readflashDataBuf[start_i++].i_value;//开启管夹阀功能


		//==20220720添加7各参数：各挡位最小值，最大值；语音开关使能
		//各挡位的最小值 最大值
		gSystemPara.u32_LevelMinAndMax[SDWeCaiJiDangWei_200][0] = readflashDataBuf[start_i++].i_value;
		gSystemPara.u32_LevelMinAndMax[SDWeCaiJiDangWei_200][1] = readflashDataBuf[start_i++].i_value;
		gSystemPara.u32_LevelMinAndMax[SDWeCaiJiDangWei_300][0] = readflashDataBuf[start_i++].i_value;
		gSystemPara.u32_LevelMinAndMax[SDWeCaiJiDangWei_300][1] = readflashDataBuf[start_i++].i_value;
		gSystemPara.u32_LevelMinAndMax[SDWeCaiJiDangWei_400][0] = readflashDataBuf[start_i++].i_value;
		gSystemPara.u32_LevelMinAndMax[SDWeCaiJiDangWei_400][1] = readflashDataBuf[start_i++].i_value;

		//语音开关
		gSystemPara.u32_50PerVoicePrintf = readflashDataBuf[start_i++].i_value;
		gSystemPara.u32_80PerVoicePrintf = readflashDataBuf[start_i++].i_value;
		gSystemPara.u32_90PerVoicePrintf = readflashDataBuf[start_i++].i_value;	
		
		gSystemPara.u32_MotorRockFre = readflashDataBuf[start_i++].i_value;	//电机摇摆频率

		gSystemPara.u32_LiuSu_Window = readflashDataBuf[start_i++].i_value;//流速窗口
		gSystemPara.u32_YaoBaiVlu_TuanCai = readflashDataBuf[start_i++].i_value;//摇摆值（团采）
		gSystemPara.u32_YaoBaiTime_TuanCai = readflashDataBuf[start_i++].i_value;//摇摆时间（团采）
		gSystemPara.u32_FastWarnFilterTime = readflashDataBuf[start_i++].i_value;//快速滤波时间
		gSystemPara.u32_SlowWarnFilterTime = readflashDataBuf[start_i++].i_value;//慢速滤波时间

		//add at 20240813
		gSystemPara.u32_CaiJiWanChengBoBaoShengGuangKongEn = readflashDataBuf[start_i++].i_value;/**< 采集完成声光控使能*/
		gSystemPara.u32_CaiJiWanChengBoBaoCiShu = readflashDataBuf[start_i++].i_value;/**< 采集完成播报次数*/
		gSystemPara.u32_CaiJiWanChengBoBaoJianGeShiJian = readflashDataBuf[start_i++].i_value;/**< 采集完成播报间隔时间*/
		gSystemPara.u32_CaiJiWanChengYaoBaiKaiGuanEn = readflashDataBuf[start_i++].i_value;/**< 采集完成摇摆开关*/
		gSystemPara.u32_LiuSuYiChangBoBaoLeiXing = readflashDataBuf[start_i++].i_value;/**< 流速异常播报类型*/
	}
}
...
void storeSysDataToFlash_3030(void)
{
	...
	start_i = end_i ;
	end_i = start_i+3;
	pInt32 = (INT32 *)&(gSystemPara.yusheWeight[SDWeCaiJiDangWei_200]);/**< 200 300 400 档预设重量 */
	for(;start_i<end_i;start_i++)
	{
		pWordInt32Float[start_i].i_value = *pInt32++;
	}
	...
	start_i = end_i ;
	end_i = start_i+1;
	pInt32 = (INT32 *)&(gSystemPara.u16_ml_g_bilv);/**< ml与g比率 */
	for(;start_i<end_i;start_i++)
	{
		pWordInt32Float[start_i].i_value = *pInt32++;
	}
	...
	start_i = end_i ;
	end_i = start_i+1;
	pInt32 = (INT32 *)&(gSystemPara.u16_fmqkqsj);/**< 蜂鸣器开始时间 */
	for(;start_i<end_i;start_i++)
	{
		pWordInt32Float[start_i].i_value = *pInt32++;
	}
	...
	start_i = end_i ;
	end_i = start_i+1;
	pInt32 = (INT32 *)&(gSystemPara.u16_liusu_high_warn);/**< 流速高 报警点 */
	for(;start_i<end_i;start_i++)
	{
		pWordInt32Float[start_i].i_value = *pInt32++;
	}
	...
	start_i = end_i ;
	end_i = start_i+1;
	pInt32 = (INT32 *)&(gSystemPara.u16_liusu_low_warn);/**< 流速低 报警点 */
	for(;start_i<end_i;start_i++)
	{
		pWordInt32Float[start_i].i_value = *pInt32++;
	}
	...
	start_i = end_i ;
	end_i = start_i+1;
	pInt32 = (INT32 *)&(motorCtl.motorAge);/**< 摇摆角度 */
	for(;start_i<end_i;start_i++)
	{
		pWordInt32Float[start_i].i_value = *pInt32++;
	}
	...
	start_i = end_i ;
	end_i = start_i+1;
	pInt32 = (INT32 *)&(motorCtl.tAD);/**< 摇摆时间 */
	for(;start_i<end_i;start_i++)
	{
		pWordInt32Float[start_i].i_value = *pInt32++;
	}
	...
	start_i = end_i ;
	end_i = start_i+1;
	pInt32 = (INT32 *)&(gSystemPara.u16_kaiqi_guanjiafa_gongneng);/**< 开启管夹阀功能 */
	for(;start_i<end_i;start_i++)
	{
		pWordInt32Float[start_i].i_value = *pInt32++;
	}
	...
	start_i = end_i ;
	end_i = start_i+6;
	pInt32 = (INT32 *)&(gSystemPara.u32_LevelMinAndMax[SDWeCaiJiDangWei_200][0]);/**< 各挡位最小值 最大值 */
	for(;start_i<end_i;start_i++)
	{
		pWordInt32Float[start_i].i_value = *pInt32++;
	}
	...
	start_i = end_i ;
	end_i = start_i+3;
	pInt32 = (INT32 *)&(gSystemPara.u32_50PerVoicePrintf);/**< 50% 80% 90% 语音开关 */
	for(;start_i<end_i;start_i++)
	{
		pWordInt32Float[start_i].i_value = *pInt32++;
	}
	...
	start_i = end_i ;
	end_i = start_i+1;
	pInt32 = (INT32 *)&(gSystemPara.u32_MotorRockFre);/**< 电机摇摆频率 */
	for(;start_i<end_i;start_i++)
	{
		pWordInt32Float[start_i].i_value = *pInt32++;
	}
	...
	start_i = end_i ;
	end_i = start_i+1;
	pInt32 = (INT32 *)&(gSystemPara.u32_LiuSu_Window);/**< 流速窗口 */
	for(;start_i<end_i;start_i++)
	{
		pWordInt32Float[start_i].i_value = *pInt32++;
	}
	...
	start_i = end_i ;
	end_i = start_i+1;
	pInt32 = (INT32 *)&(gSystemPara.u32_YaoBaiVlu_TuanCai);/**< 摇摆值（团采） */
	for(;start_i<end_i;start_i++)
	{
		pWordInt32Float[start_i].i_value = *pInt32++;
	}
	...
	start_i = end_i ;
	end_i = start_i+1;
	pInt32 = (INT32 *)&(gSystemPara.u32_YaoBaiTime_TuanCai);/**< 摇摆时间（团采） */
	for(;start_i<end_i;start_i++)
	{
		pWordInt32Float[start_i].i_value = *pInt32++;
	}
	...
	start_i = end_i ;
	end_i = start_i+1;
	pInt32 = (INT32 *)&(gSystemPara.u32_FastWarnFilterTime);/**< 快速滤波时间 */
	for(;start_i<end_i;start_i++)
	{
		pWordInt32Float[start_i].i_value = *pInt32++;
	}
	...
	start_i = end_i ;
	end_i = start_i+1;
	pInt32 = (INT32 *)&(gSystemPara.u32_SlowWarnFilterTime);/**< 慢速滤波时间 */
	for(;start_i<end_i;start_i++)
	{
		pWordInt32Float[start_i].i_value = *pInt32++;
	}
	...
	start_i = end_i ;
	end_i = start_i+1;
	pInt32 = (INT32 *)&(gSystemPara.u32_CaiJiWanChengBoBaoShengGuangKongEn);/**< 采集完成声光控使能 */
	for(;start_i<end_i;start_i++)
	{
		pWordInt32Float[start_i].i_value = *pInt32++;
	}
	...
	start_i = end_i ;
	end_i = start_i+1;
	pInt32 = (INT32 *)&(gSystemPara.u32_CaiJiWanChengBoBaoCiShu);/**< 采集完成播报次数 */
	for(;start_i<end_i;start_i++)
	{
		pWordInt32Float[start_i].i_value = *pInt32++;
	}
	...
	start_i = end_i ;
	end_i = start_i+1;
	pInt32 = (INT32 *)&(gSystemPara.u32_CaiJiWanChengBoBaoJianGeShiJian);/**< 采集完成播报间隔时间 */
	for(;start_i<end_i;start_i++)
	{
		pWordInt32Float[start_i].i_value = *pInt32++;
	}
	...
	start_i = end_i ;
	end_i = start_i+1;
	pInt32 = (INT32 *)&(gSystemPara.u32_CaiJiWanChengYaoBaiKaiGuanEn);/**< 采集完成摇摆开关 */
	for(;start_i<end_i;start_i++)
	{
		pWordInt32Float[start_i].i_value = *pInt32++;
	}
	...
	start_i = end_i ;
	end_i = start_i+1;
	pInt32 = (INT32 *)&(gSystemPara.u32_LiuSuYiChangBoBaoLeiXing);/**< 流速异常播报类型 */
	for(;start_i<end_i;start_i++)
	{
		pWordInt32Float[start_i].i_value = *pInt32++;
	}
