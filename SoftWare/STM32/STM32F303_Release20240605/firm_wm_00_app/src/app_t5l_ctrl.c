...
void screenSDWe_CaiJiYuYin_Handle(void)
{
	...
	{
		#if 0
		if(FALSE == g_T5L.liusuSlowWarnPrintf)
		{
			//可以打印
			u8_liusuLowPrintfEn = TRUE;
		}
		else
		{
			if(g_T5L.liusuSlowWarnDelay > 0)
			{
				g_T5L.liusuSlowWarnDelay--;
				u8_liusuLowPrintfEn = FALSE;
			}
			if(0 == g_T5L.liusuSlowWarnDelay)
			{
				//可以打印
				u8_liusuLowPrintfEn = TRUE;
			}
		...
		if(TRUE == u8_liusuLowPrintfEn)
		{
			if(TRUE == screenVoicePrintf_Handle(SDWEVoicePrintf_Warn_LiuSu_Low))
			{
				...
				g_T5L.liusuError = 0 ;
				g_T5L.liusuSlowWarnPrintf = TRUE;
				g_T5L.liusuSlowWarnDelay = gSystemPara.u32_SlowWarnFilterTime;
			}
		}
		#else
			if(TRUE == screenVoicePrintf_Handle(SDWEVoicePrintf_Warn_LiuSu_Low))
...
void screenSDWe_LiuSuCaculate(UINT32 cycleAvg,UINT32 cycleCurFast,UINT32 cycleCurSlow)
{
	...
	//else
	{
		...
		//计算采集时间对应的平均流速
		if((u32_SampleTotalTimes != 0) &&(u32_MiddleWeightAvgCaculateTick%500 == 0))
		{
