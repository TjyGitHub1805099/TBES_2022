...
#include "hal_uart.h"
  ...
  // UART 外设
UsartType* UART_PORT[] = { UART1_PORT, UART2_PORT, UART3_PORT };
const UINT32 UART_PORT_CLK[] = { UART1_PORT_CLK, UART2_PORT_CLK, UART3_PORT_CLK };
const INT8 UART_PORT_IRQn[] = { UART1_PORT_IRQn, UART2_PORT_IRQn, UART3_PORT_IRQn };
const UINT8 UART_PORT_GPIO_AF[] = { UART1_PORT_GPIO_AF, UART2_PORT_GPIO_AF, UART3_PORT_GPIO_AF };
// UART 发送引脚
GpioType* UART_TX_GPIO_PORT[] = { UART1_TX_GPIO_PORT, UART2_TX_GPIO_PORT, UART3_TX_GPIO_PORT };
const UINT32 UART_TX_GPIO_CLK[] = { UART1_TX_GPIO_CLK, UART2_TX_GPIO_CLK, UART3_TX_GPIO_CLK };
const UINT16 UART_TX_GPIO_PIN[] = { UART1_TX_GPIO_PIN, UART2_TX_GPIO_PIN, UART3_TX_GPIO_PIN };
const UINT16 UART_TX_PIN_SOURCE[] = { UART1_TX_PIN_SOURCE, UART2_TX_PIN_SOURCE, UART3_TX_PIN_SOURCE };
// UART 接收引脚
GpioType* UART_RX_GPIO_PORT[] = { UART1_RX_GPIO_PORT, UART2_RX_GPIO_PORT, UART3_RX_GPIO_PORT };
const UINT32 UART_RX_GPIO_CLK[] = { UART1_RX_GPIO_CLK, UART2_RX_GPIO_CLK, UART3_RX_GPIO_CLK };
const UINT16 UART_RX_GPIO_PIN[] = { UART1_RX_GPIO_PIN, UART2_RX_GPIO_PIN, UART3_RX_GPIO_PIN };
const UINT16 UART_RX_PIN_SOURCE[] = { UART1_RX_PIN_SOURCE, UART2_RX_PIN_SOURCE, UART3_RX_PIN_SOURCE };
// UART 发送使能引脚
GpioType* UART_DEA_GPIO_PORT[] = { UART1_DEA_GPIO_PORT, UART2_DEA_GPIO_PORT , UART3_DEA_GPIO_PORT };
const UINT32 UART_DEA_GPIO_CLK[] = { UART1_DEA_GPIO_CLK, UART2_DEA_GPIO_CLK , UART3_DEA_GPIO_CLK };
const UINT16 UART_DEA_GPIO_PIN[] = { UART1_DEA_GPIO_PIN, UART2_DEA_GPIO_PIN , UART3_DEA_GPIO_PIN };
//// UART 接收使能引脚
...
// UART 发送DMA
DmaType* UART_TX_DMA[] = { UART1_TX_DMA, UART2_TX_DMA, UART3_TX_DMA };
const UINT32 UART_TX_DMA_CLK[] = { UART1_TX_DMA_CLK, UART2_TX_DMA_CLK, UART3_TX_DMA_CLK };
DmaChannelType* UART_TX_DMA_CHANNEL[] = { UART1_TX_DMA_CHANNEL, UART2_TX_DMA_CHANNEL, UART3_TX_DMA_CHANNEL };
const INT8 UART_TX_DMA_IRQn[] = { UART1_TX_DMA_IRQn, UART2_TX_DMA_IRQn, UART3_TX_DMA_IRQn };
const UINT32 UART_TX_DMA_IT_TCIF[] = { UART1_TX_DMA_IT_TCIF, UART2_TX_DMA_IT_TCIF, UART3_TX_DMA_IT_TCIF };
//const UINT32 UART_TX_DMA_IT_GIF[] = { UART1_TX_DMA_IT_GIF };
//const UINT32 UART_TX_DMA_FLAG_GIF[] = { UART1_TX_DMA_FLAG_GIF };
// UART 接收DMA
DmaType* UART_RX_DMA[] = { UART1_RX_DMA, UART2_RX_DMA, UART3_RX_DMA };
DmaChannelType* UART_RX_DMA_CHANNEL[] = { UART1_RX_DMA_CHANNEL, UART2_RX_DMA_CHANNEL, UART3_RX_DMA_CHANNEL };
const UINT32 UART_RX_DMA_CLK[] = { UART1_RX_DMA_CLK, UART2_RX_DMA_CLK, UART3_RX_DMA_CLK };
...
* @retval 无
*/
void hal_uart_set_tx_mode( UartDeviceType *pUartDevice )
{
...
* @retval 无
*/
void hal_uart_set_rx_mode( UartDeviceType *pUartDevice )
{
...
* @retval 无
*/
void drv_uart_tx_byte( UsartType *pUart, UINT8 Data )
{
	pUart->TDR.all = Data;
}
...
* @retval 接收到的数据.
*/
UINT8  drv_uart_rx_byte( UsartType *pUart )
{
	UsartRdrType l_Rdr;
...
/**
* @brief  串口1(扫码枪)中断服务函数
* @retval 无
*/
void uart_smq_isr( void )
{
	UsartIsrType l_Sr;
	l_Sr.all = UART_PORT[UART_SMQ]->ISR.all;

	if( l_Sr.bit.RTOF == 1 )		//接收超时中断
	{
		drv_uart_clear_it_pending_bit( UART_PORT[UART_SMQ], USART_IT_RTO );
		drv_dma_stream_disable( UART_RX_DMA_CHANNEL[UART_SMQ] );

		// 处理数据
		*g_UartDevice[UART_SMQ].pRxLength = g_UartDevice[UART_SMQ].RxBytesMax - drv_dma_stream_get_left_length( UART_RX_DMA_CHANNEL[UART_SMQ] );
		*(g_UartDevice[UART_SMQ].pRxFinishFlag) = 1;
		//      hal_3048_msg_packet_process( &g_UartDevice[UART_COM] );
			  //*g_UartDevice[ UART_COM ].pRxFinishFlag = 0;
			  //*g_UartDevice[ UART_COM ].pRxLength = 0;

			  // 重新设置接收地址和长度，并启动DMA接收
		drv_dma_set_mem_addr_length( UART_RX_DMA_CHANNEL[UART_SMQ], (UINT32)g_UartDevice[UART_SMQ].pRxBuffer, g_UartDevice[UART_SMQ].RxBytesMax );
		drv_dma_stream_enable( UART_RX_DMA_CHANNEL[UART_SMQ] );
	}
	else if( l_Sr.bit.RXNE == 1 )	// 接收数据中断
	{
		drv_uart_clear_it_pending_bit( UART_PORT[UART_SMQ], USART_IT_RXNE );
		g_UartDevice[UART_SMQ].pRxBuffer[*g_UartDevice[UART_SMQ].pRxLength++] = drv_uart_rx_byte( UART_PORT[UART_SMQ] );
	...
	else if( (l_Sr.bit.TXE == 1) || (l_Sr.bit.TC == 1) )
	{
		if( l_Sr.bit.TC == 1 )
		{
			drv_uart_clear_it_pending_bit( UART_PORT[UART_SMQ], USART_IT_TC );
			g_UartDevice[UART_SMQ].TxBusyFlag = 0;
			// 方式完成，根据配置切换到接收模式
			if( (g_UartDevice[UART_SMQ].LinkType == UART_LINK_RX_TX_HALF) || (g_UartDevice[UART_SMQ].LinkType == UART_LINK_RX_TX_HALF_ENABE) )
			{
				hal_uart_set_rx_mode( &(g_UartDevice[UART_SMQ]) );
			}
		}
		else if( l_Sr.bit.TXE == 1 )
		{
			drv_uart_clear_it_pending_bit( UART_PORT[UART_SMQ], USART_IT_TXE );
			if( g_UartDevice[UART_SMQ].TxCounter >= g_UartDevice[UART_SMQ].TxLength )
			{
				drv_uart_it_disable( UART_PORT[UART_SMQ], USART_IT_TXE );
				// 清发送完成中断标志，并打开发送完成中断
				drv_uart_clear_it_pending_bit( UART_PORT[UART_SMQ], USART_IT_TC );
				drv_uart_it_enable( UART_PORT[UART_SMQ], USART_IT_TC );
			}
			else
			{
				drv_uart_tx_byte( UART_PORT[UART_SMQ], g_UartDevice[UART_SMQ].pTxBuffer[g_UartDevice[UART_SMQ].TxCounter++] );
			}
		}
	}
	...
	{
		//错误处理
		hal_uart_port_init( &g_UartDevice[UART_SMQ] );
	}
}

/**
* @brief  串口1(扫码枪)DMA发送中断服务函数
* @retval 无
*/
void uart_smq_tx_dma_isr()
{
	if( drv_dma_stream_get_it_status( UART_TX_DMA_CHANNEL[UART_SMQ], UART_TX_DMA_IT_TCIF[UART_SMQ] ) )
	{
		drv_dma_stream_disable( UART_TX_DMA_CHANNEL[UART_SMQ] );
		drv_uart_it_enable( UART_PORT[UART_SMQ], USART_IT_TC );
		drv_dma_stream_clear_it_pending_bit( UART_TX_DMA_IT_TCIF[UART_SMQ] );
	}
}
/**
* @brief  串口1(扫码枪)DMA接收中断服务函数
* @retval 无
*/
void uart_smq_rx_dma_isr()
{
	if( drv_dma_stream_get_it_status( UART_TX_DMA_CHANNEL[UART_SMQ], UART_TX_DMA_IT_TCIF[UART_SMQ] ) )
	{
		drv_dma_stream_disable( UART_TX_DMA_CHANNEL[UART_SMQ] );
...
/**
* @brief  串口2(中显屏)中断服务函数
* @retval 无
*/
void uart_zxp_isr( void )
{
	UsartIsrType l_Sr;
	l_Sr.all = UART_PORT[UART_ZXP]->ISR.all;

	if( l_Sr.bit.RTOF == 1 )		//接收超时中断
	{
		drv_uart_clear_it_pending_bit( UART_PORT[UART_ZXP], USART_IT_RTO );
		drv_dma_stream_disable( UART_RX_DMA_CHANNEL[UART_ZXP] );

		// 处理数据
		*g_UartDevice[UART_ZXP].pRxLength = g_UartDevice[UART_ZXP].RxBytesMax - drv_dma_stream_get_left_length( UART_RX_DMA_CHANNEL[UART_ZXP] );
		*(g_UartDevice[UART_ZXP].pRxFinishFlag) = 1;
		//      hal_3048_msg_packet_process( &g_UartDevice[UART_COM] );
			  //*g_UartDevice[ UART_COM ].pRxFinishFlag = 0;
			  //*g_UartDevice[ UART_COM ].pRxLength = 0;

			  // 重新设置接收地址和长度，并启动DMA接收
		drv_dma_set_mem_addr_length( UART_RX_DMA_CHANNEL[UART_ZXP], (UINT32)g_UartDevice[UART_ZXP].pRxBuffer, g_UartDevice[UART_ZXP].RxBytesMax );
		drv_dma_stream_enable( UART_RX_DMA_CHANNEL[UART_ZXP] );
	}
	else if( l_Sr.bit.RXNE == 1 )	// 接收数据中断
	{
		drv_uart_clear_it_pending_bit( UART_PORT[UART_ZXP], USART_IT_RXNE );
		g_UartDevice[UART_ZXP].pRxBuffer[*g_UartDevice[UART_ZXP].pRxLength++] = drv_uart_rx_byte( UART_PORT[UART_ZXP] );
	...
	else if( (l_Sr.bit.TXE == 1) || (l_Sr.bit.TC == 1) )
	{
		if( l_Sr.bit.TC == 1 )
		{
			drv_uart_clear_it_pending_bit( UART_PORT[UART_ZXP], USART_IT_TC );
			g_UartDevice[UART_ZXP].TxBusyFlag = 0;
			// 方式完成，根据配置切换到接收模式
			if( (g_UartDevice[UART_ZXP].LinkType == UART_LINK_RX_TX_HALF) || (g_UartDevice[UART_ZXP].LinkType == UART_LINK_RX_TX_HALF_ENABE) )
			{
				hal_uart_set_rx_mode( &(g_UartDevice[UART_ZXP]) );
			}
		}
		else if( l_Sr.bit.TXE == 1 )
		{
			drv_uart_clear_it_pending_bit( UART_PORT[UART_ZXP], USART_IT_TXE );
			if( g_UartDevice[UART_ZXP].TxCounter >= g_UartDevice[UART_ZXP].TxLength )
			{
				drv_uart_it_disable( UART_PORT[UART_ZXP], USART_IT_TXE );
				// 清发送完成中断标志，并打开发送完成中断
				drv_uart_clear_it_pending_bit( UART_PORT[UART_ZXP], USART_IT_TC );
				drv_uart_it_enable( UART_PORT[UART_ZXP], USART_IT_TC );
			}
			else
			{
				drv_uart_tx_byte( UART_PORT[UART_ZXP], g_UartDevice[UART_ZXP].pTxBuffer[g_UartDevice[UART_ZXP].TxCounter++] );
			}
		}
	}
	else if( (l_Sr.bit.PE == 1) || (l_Sr.bit.FE == 1) || (l_Sr.bit.NF == 1) || (l_Sr.bit.ORE == 1) || (l_Sr.bit.LBDF == 1) )
	{
		//错误处理
		hal_uart_port_init( &g_UartDevice[UART_ZXP] );
	}
}

/**
* @brief  串口2(中显屏)DMA发送中断服务函数
* @retval 无
*/
void uart_zxp_tx_dma_isr()
{
	if( drv_dma_stream_get_it_status( UART_TX_DMA_CHANNEL[UART_ZXP], UART_TX_DMA_IT_TCIF[UART_ZXP] ) )
	{
		drv_dma_stream_disable( UART_TX_DMA_CHANNEL[UART_ZXP] );
		drv_uart_it_enable( UART_PORT[UART_ZXP], USART_IT_TC );
		drv_dma_stream_clear_it_pending_bit( UART_TX_DMA_IT_TCIF[UART_ZXP] );
	}
}
/**
* @brief  串口2(中显屏)DMA接收中断服务函数
* @retval 无
*/
void uart_zxp_rx_dma_isr()
{
	if( drv_dma_stream_get_it_status( UART_TX_DMA_CHANNEL[UART_ZXP], UART_TX_DMA_IT_TCIF[UART_ZXP] ) )
	{
		drv_dma_stream_disable( UART_TX_DMA_CHANNEL[UART_ZXP] );
...
/**
* @brief  串口3(无线模块)中断服务函数
* @retval 无
*/
void uart_wxm_isr( void )
{
	UsartIsrType l_Sr;
	l_Sr.all = UART_PORT[UART_WXM]->ISR.all;

	if( l_Sr.bit.RTOF == 1 )		//接收超时中断
	{
		drv_uart_clear_it_pending_bit( UART_PORT[UART_WXM], USART_IT_RTO );
		drv_dma_stream_disable( UART_RX_DMA_CHANNEL[UART_WXM] );

		// 处理数据
		*g_UartDevice[UART_WXM].pRxLength = g_UartDevice[UART_WXM].RxBytesMax - drv_dma_stream_get_left_length( UART_RX_DMA_CHANNEL[UART_WXM] );
		*(g_UartDevice[UART_WXM].pRxFinishFlag) = 1;
		//      hal_3048_msg_packet_process( &g_UartDevice[UART_COM] );
			  //*g_UartDevice[ UART_COM ].pRxFinishFlag = 0;
			  //*g_UartDevice[ UART_COM ].pRxLength = 0;

			  // 重新设置接收地址和长度，并启动DMA接收
		drv_dma_set_mem_addr_length( UART_RX_DMA_CHANNEL[UART_WXM], (UINT32)g_UartDevice[UART_WXM].pRxBuffer, g_UartDevice[UART_WXM].RxBytesMax );
		drv_dma_stream_enable( UART_RX_DMA_CHANNEL[UART_WXM] );
	}
	else if( l_Sr.bit.RXNE == 1 )	// 接收数据中断
	{
		drv_uart_clear_it_pending_bit( UART_PORT[UART_WXM], USART_IT_RXNE );
		g_UartDevice[UART_WXM].pRxBuffer[*g_UartDevice[UART_WXM].pRxLength++] = drv_uart_rx_byte( UART_PORT[UART_WXM] );
	...
	else if( (l_Sr.bit.TXE == 1) || (l_Sr.bit.TC == 1) )
	{
		if( l_Sr.bit.TC == 1 )
		{
			drv_uart_clear_it_pending_bit( UART_PORT[UART_WXM], USART_IT_TC );
			g_UartDevice[UART_WXM].TxBusyFlag = 0;
			// 方式完成，根据配置切换到接收模式
			if( (g_UartDevice[UART_WXM].LinkType == UART_LINK_RX_TX_HALF) || (g_UartDevice[UART_WXM].LinkType == UART_LINK_RX_TX_HALF_ENABE) )
			{
				hal_uart_set_rx_mode( &(g_UartDevice[UART_WXM]) );
			}
		}
		else if( l_Sr.bit.TXE == 1 )
		{
			drv_uart_clear_it_pending_bit( UART_PORT[UART_WXM], USART_IT_TXE );
			if( g_UartDevice[UART_WXM].TxCounter >= g_UartDevice[UART_WXM].TxLength )
			{
				drv_uart_it_disable( UART_PORT[UART_WXM], USART_IT_TXE );
				// 清发送完成中断标志，并打开发送完成中断
				drv_uart_clear_it_pending_bit( UART_PORT[UART_WXM], USART_IT_TC );
				drv_uart_it_enable( UART_PORT[UART_WXM], USART_IT_TC );
			}
			else
			{
				drv_uart_tx_byte( UART_PORT[UART_WXM], g_UartDevice[UART_WXM].pTxBuffer[g_UartDevice[UART_WXM].TxCounter++] );
			}
		}
	}
	else if( (l_Sr.bit.PE == 1) || (l_Sr.bit.FE == 1) || (l_Sr.bit.NF == 1) || (l_Sr.bit.ORE == 1) || (l_Sr.bit.LBDF == 1) )
	{
		//错误处理
		hal_uart_port_init( &g_UartDevice[UART_WXM] );
	}
}

/**
* @brief  串口3(无线模块)DMA发送中断服务函数
* @retval 无
*/
void uart_wxm_tx_dma_isr()
{
	if( drv_dma_stream_get_it_status( UART_TX_DMA_CHANNEL[UART_WXM], UART_TX_DMA_IT_TCIF[UART_WXM] ) )
	{
		drv_dma_stream_disable( UART_TX_DMA_CHANNEL[UART_WXM] );
		drv_uart_it_enable( UART_PORT[UART_WXM], USART_IT_TC );
		drv_dma_stream_clear_it_pending_bit( UART_TX_DMA_IT_TCIF[UART_WXM] );
	}
}
/**
* @brief  串口3(无线模块)DMA接收中断服务函数
* @retval 无
*/
void uart_wxm_rx_dma_isr()
{
	if( drv_dma_stream_get_it_status( UART_TX_DMA_CHANNEL[UART_WXM], UART_TX_DMA_IT_TCIF[UART_WXM] ) )
	{
		drv_dma_stream_disable( UART_TX_DMA_CHANNEL[UART_WXM] );
