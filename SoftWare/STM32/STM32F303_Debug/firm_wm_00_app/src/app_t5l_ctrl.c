...
{
	static UINT32 u32_SampleTotalTimes = 0 ,u32_JudgedTickArray[6] , entTimes = 0;
	...
	UINT8 rand_100 = 0;
	static float f_liusu_PlusRrand = 0;
	...
	//else
	{
		...
		else
		{
			...
			g_T5L.liusuErrorCycleHandle = 0 ;//拿下来后不播报异常
			if(SDWeCurPage_ShiShiJieMian != g_T5L.curPage)
			{
		...
		if((entTimes % 500) == 0)
		{
			//随机数
			rand_100 = rand()%100;
			f_liusu_PlusRrand = rand_100;
			f_liusu_PlusRrand /=100;
			f_liusu_PlusRrand *=(0.1*g_T5L.liusuCur);
			f_liusu_PlusRrand += g_T5L.liusuCur;
			SET_SDWE_CUR_CYCLE_DATA(SDWE_CYCLE_DATA_LIUSU,(INT16)(f_liusu_PlusRrand));
		}
	}
}
...
void screenSDWe_CycleDataSend(UINT16 cycle)
{
	...
	else
	{
		...
		//特殊处理 保证开始界面显示纯重量
			...
			{
				//1.保持住空袋重量 进而纯重量也就保持住了 总重量和皮重 在右上角
				//2.拿下袋子后 设置当前重量 = 0 ,如果在放回去 则进行算数运算
				if(weight < GET_SDWE_CUR_CYCLE_DATA(SDWE_CYCLE_DATA_KONGDAI_WEIGHT))
				{
					SET_SDWE_CUR_CYCLE_DATA(SDWE_CYCLE_DATA_KONGDAI_WEIGHT,0);
				}
				else if(weight > (GET_SDWE_CUR_CYCLE_DATA(SDWE_CYCLE_DATA_KONGDAI_WEIGHT) + 0.2*g_T5L.curCaiJiDangWeiWeight))
				{
					//袋子拿下在放回 , 重新设置空袋重量 , 还是显示上次的纯重
					SET_SDWE_CUR_CYCLE_DATA(SDWE_CYCLE_DATA_KONGDAI_WEIGHT,g_T5L.kongDaiRecodeWeight);
				}
			}
			...
			case 0://数据准备
				...
				else
				{
					...
					//判断托盘重量
						...
						kongDaiWeight_Matched = FALSE;
					#endif
					if(FALSE == kongDaiWeight_Matched)
					{
						gotoPage = SDWeCurPage_ShiShiJieMian;
					}
					dangwei = 0;
...
void screenSDWe_TxFunction(void)
{
	...
	switch(pScreen->sdweCtlStep)
	{
		...
		case SDWeCtlStep_CycleHandle:
			...
			else if((SDWeCurPage_TuanCai == pScreen->curPage) && 
					((SYS_CTL_EVENT_VALID == pScreen->runContrl[SYS_CTL_REG_EVENT_INDEX]) ||
			 		 (TRUE == key_EventGet(TBES_START_MODLE_CHOICE))))
			{
				if(TRUE == runContrl_Handle(pScreen->runContrl[SYS_CTL_REG_STATUS_INDEX]))
					 ...
					 (TRUE == key_EventGet(TBES_400_MODLE_CHOICE))))
			{
				...
				if((pScreen->cycleData[SDWE_CYCLE_DATA_ARR_INDEX_CUR][SDWE_CYCLE_DATA_TOTAL_WEIGHT] > gSystemPara.zeroRange) ||
				   ((g_T5L.enterRealTimeSetContrl[SYS_CTL_REG_STATUS_INDEX] == 300) || (TRUE == key_EventGet(TBES_300_MODLE_CHOICE))))
				{
			...
			else if((TRUE == pScreen->sdweJumpToHomePage))//从其他跳转至开始界面
			{
				...
				{
					g_T5L.curPage = SDWeCurPage_KaiShiJieMian;
					g_T5L.fromShiShiToStartPage = FALSE;
